...
| ~~**P1**~~ | ~~**Astro config mismatch**~~ - ~~`output: 'server'` but page has `prerender = true`~~ | ~~`astro.config.mjs` (line 21), `src/pages/index.astro` (line 3)~~ | ~~**Performance:** Conflicting configs cause confusion. Should be `output: 'static'` for SSG or remove `prerender` for SSR. Current setup works but is misleading.~~ | ~~S (30m)~~ | ~~Low~~ | ✅ **COMPLETED** (2025-01-27) - Changed to `output: 'static'` to match actual usage pattern (all pages prerender, API routes opt-out) |
